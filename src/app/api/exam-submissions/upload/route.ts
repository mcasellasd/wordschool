...
export async function POST(request: NextRequest) {
  try {
    ...
    // TODO: Aquí s'afegiria la lògica per:
    // 1. Crear registre a la base de dades
    // 2. Iniciar processament OCR de forma asíncrona
    // 3. Retornar submissionId

    // Simulació: retornar resposta amb submissionId temporal
    const submissionId = `sub-${timestamp}-${randomString}`;

    // En producció, aquí es processaria l'OCR de forma asíncrona
    // await processOCRAsync(submissionId, filePath, examId, studentId, level);
